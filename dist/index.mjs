...
import { ApiError, LokaliseApi, LokaliseApiOAuth } from "@lokalise/node-api";
import { logWithColor, logWithLevel } from "kliedz";

//#region lib/errors/LokaliseError.ts
/**
...
var LokaliseError = class extends Error {
	...
	*/
	toString() {
		...
		if (this.code) baseMessage += ` (Code: ${this.code})`;
		if (this.details) {
...
//#endregion
//#region lib/services/LokaliseFileExchange.ts
/**
* A utility class for exchanging files with the Lokalise API.
*/
var LokaliseFileExchange = class LokaliseFileExchange {
	/**
	...
	*/
	...
	static isFinishedStatus(status) {
		return status != null && LokaliseFileExchange.FINISHED_STATUSES.includes(status);
	}
	/**
	...
	*/
	constructor(clientConfig, { projectId, useOAuth2 = false, retryParams, logThreshold = "info", logColor = true }) {
		this.projectId = projectId;
		this.logThreshold = logThreshold;
		this.logger = this.chooseLogger(logColor);
		this.retryParams = this.buildRetryParams(retryParams);
		this.validateParams();
		const apiConfig = this.buildLokaliseClientConfig(clientConfig, logThreshold);
	...
	*/
	async withExponentialBackoff(operation) {
		...
		this.logMsg("debug", `Running operation with exponential backoff; max retries: ${maxRetries}`);
		for (let attempt = 1; attempt <= maxRetries + 1; attempt++) try {
			...
			if (error instanceof ApiError && this.isRetryable(error)) {
				...
				if (attempt === maxRetries + 1) throw new LokaliseError(`Maximum retries reached: ${error.message}`, error.code, error.details);
			...
			} else if (error instanceof ApiError) throw new LokaliseError(error.message, error.code, error.details);
			else throw error;
		}
	...
	*/
	async pollProcesses(processes, initialWaitTime, maxWaitTime, concurrency = LokaliseFileExchange.maxConcurrentProcesses) {
		this.logMsg("debug", `Start polling processes. Total processes count: ${processes.length}`);
		const startTime = Date.now();
		const { processMap, pendingProcessIds } = this.initializePollingState(processes);
		await this.runPollingLoop(processMap, pendingProcessIds, startTime, initialWaitTime, maxWaitTime, concurrency);
		if (pendingProcessIds.size > 0) await this.refreshRemainingProcesses(processMap, pendingProcessIds, concurrency);
	...
	*/
	initializePollingState(processes) {
		...
		for (const p of processes) {
			if (p.status) this.logMsg("debug", `Process ID: ${p.process_id}, status: ${p.status}`);
			else this.logMsg("debug", `Process ID: ${p.process_id}, status is missing`);
			processMap.set(p.process_id, p);
			if (LokaliseFileExchange.isPendingStatus(p.status)) pendingProcessIds.add(p.process_id);
		}
		return {
			processMap,
			pendingProcessIds
	...
	async runPollingLoop(processMap, pendingProcessIds, startTime, initialWaitTime, maxWaitTime, concurrency) {
		...
		while (pendingProcessIds.size > 0 && Date.now() - startTime < maxWaitTime) {
			...
			if (!didFastFollow && [...processMap.values()].some((p) => p.status == null)) {
				this.logMsg("debug", "Fast-follow: some statuses missing, quick recheck in 200ms");
			...
			for (const { id, process } of batch) {
				...
				if (LokaliseFileExchange.isFinishedStatus(process.status)) {
					this.logMsg("debug", `Process ${id} completed with status=${process.status}.`);
			...
			const remaining = maxWaitTime - (Date.now() - startTime);
			if (remaining <= 0) {
				this.logMsg("debug", "Time budget exhausted, stopping polling without extra sleep.");
				break;
			}
			const sleepMs = Math.min(waitTime, remaining);
			this.logMsg("debug", `Waiting ${sleepMs}...`);
			await LokaliseFileExchange.sleep(sleepMs);
			waitTime = Math.min(waitTime * 2, Math.max(0, maxWaitTime - (Date.now() - startTime)));
		}
	}
	/**
	...
	*/
	async refreshRemainingProcesses(processMap, pendingProcessIds, concurrency) {
		this.logMsg("debug", `Final refresh for ${pendingProcessIds.size} pending processes before return...`);
		const finalBatch = await this.fetchProcessesBatch([...pendingProcessIds], concurrency);
		for (const { id, process } of finalBatch) if (process) processMap.set(id, process);
	}
	/**
	...
	*/
	async getUpdatedProcess(processId) {
		...
		if (updatedProcess.status) this.logMsg("debug", `Process ID: ${updatedProcess.process_id}, status: ${updatedProcess.status}`);
		else this.logMsg("debug", `Process ID: ${updatedProcess.process_id}, status is missing`);
	...
	*/
	validateParams() {
		if (!this.projectId || typeof this.projectId !== "string") throw new LokaliseError("Invalid or missing Project ID.");
		const { maxRetries, initialSleepTime, jitterRatio } = this.retryParams;
		if (maxRetries < 0) throw new LokaliseError("maxRetries must be greater than or equal to zero.");
		if (initialSleepTime <= 0) throw new LokaliseError("initialSleepTime must be a positive value.");
		if (jitterRatio < 0 || jitterRatio > 1) throw new LokaliseError("jitterRatio must be between 0 and 1.");
	}
	/**
	...
	async runWithConcurrencyLimit(items, limit, worker) {
		...
		const workers = new Array(Math.min(limit, items.length)).fill(null).map(async () => {
			while (true) {
				...
				if (item === void 0) throw new Error(`Missing item at index ${idx}`);
	...
	*/
	async fetchProcessesBatch(processIds, concurrency = LokaliseFileExchange.maxConcurrentProcesses) {
		return this.runWithConcurrencyLimit(processIds, concurrency, async (id) => {
			try {
				return {
					id,
					process: await this.getUpdatedProcess(id)
				};
			} catch (error) {
	...
	*/
	calculateSleepMs(retryParams, attempt) {
		...
		return base + (maxJitter > 0 ? Math.floor(rng() * maxJitter) : 0);
	}
	/**
...
//#endregion
//#region lib/services/LokaliseDownload.ts
/**
* Handles downloading and extracting translation files from Lokalise.
*/
var LokaliseDownload = class LokaliseDownload extends LokaliseFileExchange {
	...
	*/
	async downloadTranslations({ downloadFileParams, extractParams = {}, processDownloadFileParams }) {
		...
		const translationsBundleURL = await this.fetchTranslationBundleURL(downloadFileParams, processParams);
		const zipFilePath = await this.downloadZip(translationsBundleURL, processParams.bundleDownloadTimeout);
		await this.processZip(zipFilePath, path.resolve(extractParams.outputDir ?? "./"));
	}
	/**
	...
	async unpackZip(zipFilePath, outputDir) {
		return new Promise((resolve, reject) => {
			yauzl.open(zipFilePath, { lazyEntries: true }, (err, zipfile) => {
				if (err) return reject(new LokaliseError(`Failed to open ZIP file at ${zipFilePath}: ${err.message}`));
				zipfile.readEntry();
	...
	*/
	async downloadZip(url, downloadTimeout = 0) {
		...
		const response = await this.fetchZipResponse(bundleURL, signal, downloadTimeout);
	...
	async fetchZipResponse(bundleURL, signal, downloadTimeout) {
		try {
			...
			if (err instanceof Error) {
				if (err.name === "TimeoutError") throw new LokaliseError(`Request timed out after ${downloadTimeout}ms`, 408, { reason: "timeout" });
				throw new LokaliseError(err.message, 500, { reason: "network or fetch error" });
			}
			/* v8 ignore start */
			throw new LokaliseError("An unknown error occurred. This might indicate a bug.", 500, { reason: String(err) });
		}
	}
	/**
	...
	*/
	getZipResponseBody(response, originalUrl) {
		if (!response.ok) throw new LokaliseError(`Failed to download ZIP file: ${response.statusText} (${response.status})`);
		const body = response.body;
		if (!body) throw new LokaliseError(`Response body is null. Cannot download ZIP file from URL: ${originalUrl}`);
		return body;
	}
	/**
	...
	*/
	async writeZipToDisk(body, tempZipPath) {
		try {
			...
			await this.streamPipeline(nodeReadable, fs.createWriteStream(tempZipPath));
		} catch (e) {
			try {
				await fs.promises.unlink(tempZipPath);
			} catch {
				this.logMsg("debug", `Stream pipeline failed and unable to remove temp path ${tempZipPath}`);
			}
			throw e;
		}
	}
	...
	*/
	async getTranslationsBundle(downloadFileParams) {
		return this.withExponentialBackoff(() => this.apiClient.files().download(this.projectId, downloadFileParams));
	}
	/**
	* Retrieves a translation bundle from Lokalise with retries and exponential backoff.
	*
	...
	*/
	async getTranslationsBundleAsync(downloadFileParams) {
		return this.withExponentialBackoff(() => this.apiClient.files().async_download(this.projectId, downloadFileParams));
	}
	/**
	...
	async handleZipEntry(entry, zipfile, outputDir) {
		...
		return new Promise((response, reject) => {
			zipfile.openReadStream(entry, (readErr, readStream) => {
				if (readErr || !readStream) return reject(new LokaliseError(`Failed to read ZIP entry: ${entry.fileName}`));
	...
	*/
	processZipEntryPath(outputDir, entryFilename) {
		...
		if (relative.startsWith("..") || path.isAbsolute(relative)) throw new LokaliseError(`Malicious ZIP entry detected: ${entryFilename}`);
		return fullPath;
	}
	/**
	...
	*/
	assertHttpUrl(value) {
		...
		if (parsed.protocol !== "http:" && parsed.protocol !== "https:") throw new LokaliseError(`Unsupported protocol in URL: ${value}`);
		return parsed;
	}
	/**
	...
	*/
	async processZip(zipFilePath, unpackTo) {
		this.logMsg("debug", `Unpacking translations from ${zipFilePath} to ${unpackTo}`);
		try {
	...
	*/
	async fetchBundleURLSync(downloadFileParams) {
		...
		return (await this.getTranslationsBundle(downloadFileParams)).bundle_url;
	}
	/**
	...
	*/
	async pollAsyncDownload(downloadProcess, initialWait, maxWait) {
		this.logMsg("debug", `Waiting for download process ID ${downloadProcess.process_id} to complete...`);
		this.logMsg("debug", `Effective waits: initial=${initialWait}ms, max=${maxWait}ms`);
		const completedProcess = (await this.pollProcesses([downloadProcess], initialWait, maxWait)).find((p) => p.process_id === downloadProcess.process_id);
		if (!completedProcess) throw new LokaliseError(`Process ${downloadProcess.process_id} not found after polling`, 500);
		if (!LokaliseFileExchange.isFinishedStatus(completedProcess.status)) throw new LokaliseError(`Download process did not finish within ${maxWait}ms${completedProcess.status ? ` (last status=${completedProcess.status})` : " (status missing)"}`, 504);
	...
	*/
	handleFinishedAsyncProcess(completedProcess) {
		...
		if (!url || typeof url !== "string") {
			this.logMsg("warn", "Process finished but details.download_url is missing or invalid", details);
			throw new LokaliseError("Lokalise returned finished process without a valid download_url", 502);
		}
		return url;
	}
	/**
	...
	*/
	handleFailedAsyncProcess(completedProcess) {
		...
		throw new LokaliseError(`Process ${completedProcess.process_id} ended with status=${completedProcess.status}` + (msg ? `: ${msg}` : ""), 502);
	}
	/**
	...
	*/
	handleUnexpectedAsyncProcess(completedProcess, maxWait) {
		...
		throw new LokaliseError(`Download process took too long to finalize; effective=${maxWait}ms`, 500);
	}
	/**
	...
	*/
	async fetchBundleURLAsync(downloadFileParams, processParams) {
		...
		const completedProcess = await this.pollAsyncDownload(downloadProcess, pollInitialWaitTime, pollMaximumWaitTime);
		this.logMsg("debug", `Download process status is ${completedProcess.status}`);
		if (completedProcess.status === "finished") return this.handleFinishedAsyncProcess(completedProcess);
		if (completedProcess.status === "failed" || completedProcess.status === "cancelled") this.handleFailedAsyncProcess(completedProcess);
		this.handleUnexpectedAsyncProcess(completedProcess, pollMaximumWaitTime);
	}
};

//#endregion
//#region lib/services/LokaliseUpload.ts
/**
* Handles uploading translation files to Lokalise.
*/
var LokaliseUpload = class LokaliseUpload extends LokaliseFileExchange {
	...
	*/
	async uploadTranslations({ uploadFileParams, collectFileParams, processUploadFileParams } = {}) {
		...
		const { processes, errors } = await this.parallelUpload(collectedFiles, uploadFileParams, processUploadFileParams);
		let completedProcesses = processes;
		this.logMsg("debug", "File uploading queued! IDs:", completedProcesses.map((p) => p.process_id));
		if (pollStatuses) {
			this.logMsg("debug", "Polling queued processes...");
			completedProcesses = await this.pollProcesses(processes, pollInitialWaitTime, pollMaximumWaitTime);
	...
	*/
	async collectFiles({ inputDirs = ["./locales"], extensions = [".*"], excludePatterns = [], recursive = true, fileNamePattern = ".*" } = {}) {
		...
		return (await this.processCollectionQueue(queue, normalizedExtensions, fileNameRegex, excludeRegexes, recursive)).sort();
	}
	/**
	...
	*/
	async uploadSingleFile(uploadParams) {
		return this.withExponentialBackoff(() => this.apiClient.files().upload(this.projectId, uploadParams));
	}
	/**
	...
	*/
	async processFile(file, projectRoot, processParams) {
		const relativePath = await this.inferRelativePath(file, projectRoot, processParams);
		const languageCode = await this.inferLanguageCode(file, relativePath, processParams);
		return {
	...
	*/
	async inferRelativePath(file, projectRoot, processParams) {
		try {
			...
			if (!fromInferer.trim()) throw new Error("Invalid filename: empty or only whitespace");
			return fromInferer;
		} catch {
			return path.posix.relative(this.toPosixPath(projectRoot), this.toPosixPath(file));
		}
	}
	/**
	...
	*/
	async inferLanguageCode(file, relativePath, processParams) {
		try {
			...
			if (!fromInferer.trim()) throw new Error("Invalid language code: empty or only whitespace");
			return fromInferer;
		} catch {
			return path.basename(relativePath).split(".").slice(-2, -1)[0] ?? "unknown";
		}
	}
	/**
	...
	*/
	async parallelUpload(files, baseUploadFileParams = {}, processParams) {
		...
		const errors = [];
		await this.runWithConcurrencyLimit(files, LokaliseUpload.maxConcurrentProcesses, async (file) => {
			try {
				const processedFileParams = await this.processFile(file, projectRoot, processParams);
	...
	*/
	async processCollectionQueue(queue, exts, nameRx, excludeRx, recursive) {
		...
		while (queue.length) {
			...
			/* v8 ignore start */
			if (!dir) {
				this.logMsg("debug", `collectFiles: received falsy dir entry (${String(dir)}). This is unexpected and might indicate a bug.`);
				continue;
			}
			/* v8 ignore stop */
	...
	*/
	handleEntry(entry, fullPath, queue, found, opts) {
		if (this.shouldExclude(fullPath, opts.excludeRx)) return;
		if (entry.isDirectory()) {
			if (opts.recursive) queue.push(fullPath);
			return;
		}
		if (entry.isFile() && this.shouldCollectFile(entry, opts.exts, opts.nameRx)) found.push(fullPath);
	}
	/**
